...
std::vector<std::pair<std::string, std::string>> parse_session_options(int argc, char ** argv) {
    ...
    for (int i = 1; i + 1 < argc; ++i) {
        ...
        if (eq == std::string::npos || eq == 0) {
            throw std::runtime_error("invalid Qwen3 ASR --session-option: " + option);
        }
...
int main(int argc, char ** argv) {
    try {
        const std::filesystem::path model_path =
            engine::tools::arg_value(argc, argv, "--model", "models/Qwen3-ASR-0.6B");
        ...
        const int max_new_tokens = engine::tools::int_arg(argc, argv, "--max-new-tokens", 512);
        const std::string default_language = engine::tools::arg_value(argc, argv, "--language", "English");
        const std::string default_context = engine::tools::arg_value(argc, argv, "--context", "");
        const std::string warmup_language = engine::tools::arg_value(argc, argv, "--warmup-language", default_language);
        const std::string warmup_context = engine::tools::arg_value(argc, argv, "--warmup-context", default_context);
        const std::filesystem::path timing_path =
            engine::tools::arg_value(argc, argv, "--timing-file", "/tmp/qwen3_asr_warm_bench_timing.log");
        ...
        load_request.family_hint = "qwen3_asr";
        ...
        if (session == nullptr) {
            throw std::runtime_error("Qwen3 ASR did not create an offline task session");
        }
        ...
        warmup_request.text_input = engine::runtime::Transcript{warmup_context, warmup_language};
        warmup_request.options["max_new_tokens"] = std::to_string(max_new_tokens);
        ...
        const auto request_contexts = split_csv_keep_empty(engine::tools::arg_value(argc, argv, "--context-sequence", ""));
        ...
        for (size_t request_index = 0; request_index < request_paths.size(); ++request_index) {
            ...
            const std::string context = request_index < request_contexts.size()
                ? request_contexts[request_index]
                : repeated_arg(argc, argv, "--request-context", request_index, default_context);
            ...
            run_request.text_input = engine::runtime::Transcript{context, language};
            run_request.options["max_new_tokens"] = std::to_string(max_new_tokens);
            ...
            std::cout << "qwen3_asr.wall_ms=" << wall_ms << "\n";
        ...
        std::cerr << "qwen3_asr_warm_bench failed: " << ex.what() << "\n";
        return 1;
    }
}
