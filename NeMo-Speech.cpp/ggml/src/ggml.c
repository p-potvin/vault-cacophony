...
static struct ggml_tensor * ggml_fused_relpos_attn_impl(
        struct ggml_context * ctx,
        struct ggml_tensor  * q,
        struct ggml_tensor  * k,
        struct ggml_tensor  * v,
    ...
    GGML_ASSERT((kv_cache == NULL) == (ring_heads == NULL));
    if (kv_cache) {
        GGML_ASSERT(cache_len > 0);
        ...
        GGML_ASSERT(ring_heads->ne[0] == q->ne[3]);
    } else {
        GGML_ASSERT(cache_len == 0);
    }
    if (mask) {
...
struct ggml_tensor * ggml_fused_relpos_attn(
        ...
        float                 scale,
        bool                  merge_heads) {
    return ggml_fused_relpos_attn_impl(
...
struct ggml_tensor * ggml_fused_relpos_attn_cached(
        ...
        bool                  merge_heads) {
    return ggml_fused_relpos_attn_impl(
        ctx, q, k, v, p, bias_u, bias_v, mask, kv_cache, slot_ids, ring_heads, cache_len, scale,
        merge_heads);
}
