#!/bin/bash
# validate-macos.sh - Validate macOS Application with embedded crispasr.xcframework using SwiftUI
...
if [ -d "${APP_PATH}/Contents/Frameworks/crispasr.framework" ]; then
...
if [ -f "${APP_PATH}/Contents/Frameworks/crispasr.framework/Versions/A/crispasr" ]; then
    echo "✅ Framework binary exists"

    # Further validate framework by checking architecture
    ARCHS=$(lipo -info "${APP_PATH}/Contents/Frameworks/crispasr.framework/Versions/A/crispasr" 2>/dev/null | grep -o "arm64\\|x86_64" | tr '\n' ' ')
    if [ -n "$ARCHS" ]; then
