...
static float* lfm2_detokenize(lfm2_audio_context* ctx, const std::vector<std::vector<int32_t>>& codes,
    ...
    //
    // The pool has to scale with the upsampled length. A fixed 512 MB fits
    // about 175 audio frames and then aborts in ggml_new_object: a 14.3 s
    // reply (179 frames, Tu = 1074) asked for 537,641,568 bytes against
    // 536,870,912 available and killed the process after generation had
    // already succeeded, which is the worst possible place to fail.
    //
    // Activations dominate and grow linearly in Tu, measured at ~977 bytes per
    // (Tu x h); 1400 leaves room without being wasteful, and the old 512 MB
    // stays as the floor for short replies.
    const size_t mem = std::max<size_t>(512ULL * 1024 * 1024,
                                        static_cast<size_t>(Tu) * static_cast<size_t>(h) * 1400ULL);
