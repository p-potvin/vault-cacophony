...
// Sampling range for the audio streams. MUST include MO2_EOA (4096), which sits
// one past the end of the acoustic codebook: sampling only MO2_AUDIO_VOCAB
// entries covers 0..4095, so end-of-audio can never be selected and generation
// runs to the step cap (~175 s of audio for a 15-token reply). The text stream
// does not have this problem because MO2_EOT (151936) is inside text_vocab_size
// (152000), which is why text terminates correctly while audio does not.
static constexpr int MO2_AUDIO_SAMPLE_VOCAB = MO2_EOA + 1;
...
static float* mo2_decode_snac(mini_omni2_context* ctx, const std::vector<std::vector<int32_t>>& audio_streams,
    ...
    // The 7 streams are layershifted, so stream s lags the others by s+1 steps.
    // The reference undoes this before deinterleaving:
    //     for i in range(7): output_list[i] = output_list[i][i + 1:]
    // Reading every stream at the same index leaves the codebooks misaligned in
    // time, which decodes to audio with a speech-shaped envelope but wrong
    // spectral content - i.e. gibberish.
    const int n_valid = n_steps - 7;
    if (n_valid <= 0)
        return nullptr;
    ...
    for (int i = 0; i < n_valid; i++) {
        c0.push_back(audio_streams[0][i + 1]); // stream 0 → codebook 0
        c1.push_back(audio_streams[1][i + 2]); // stream 1 → codebook 1
        c2.push_back(audio_streams[2][i + 3]); // stream 2 → codebook 2
        c2.push_back(audio_streams[3][i + 4]); // stream 3 → codebook 2
        c1.push_back(audio_streams[4][i + 5]); // stream 4 → codebook 1
        c2.push_back(audio_streams[5][i + 6]); // stream 5 → codebook 2
        c2.push_back(audio_streams[6][i + 7]); // stream 6 → codebook 2
    }
...
static void mo2_generate_dual(mini_omni2_context* ctx, const float* input_embeds, int T_total,
    ...
    for (int s = 0; s < 7; s++) {
        ...
        audio_toks[s] = mo2_sample_token(a_logits, MO2_AUDIO_SAMPLE_VOCAB, ctx->params.temperature);
    ...
    bool text_end = false;
    for (int step = 0; step < max_tokens - 1; step++) {
        // The 7 audio streams are layershifted, so stream 6 is the most delayed
        // and carries no meaningful content for the first few steps - an EOA
        // there is spurious. Only honour it once the streams have filled.
        if (step >= 7 && audio_toks[6] == MO2_EOA)
            break;
        if (text_tok == MO2_EOT)
            text_end = true;
...
extern "C" float* mini_omni2_synthesize(struct mini_omni2_context* ctx, const char* text, int* out_n_samples) {
    ...
    // Honour -n/--max-new-tokens here as the ASR path already does. mini-omni2's
    // audio stream terminates unreliably (persistent audio loss near the
    // termination phase destabilises EOA prediction), so an external bound is
    // the only dependable stop. ~85ms of audio per step at 24kHz.
    mo2_generate_dual(ctx, avg_emb.data(), T_total, gen_text, gen_audio,
                      ctx->max_new_tokens > 0 ? ctx->max_new_tokens : 2048);
